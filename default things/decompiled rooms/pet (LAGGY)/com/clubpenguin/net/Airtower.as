//Created by Action Script Viewer - https://www.buraks.com/asv
    class com.clubpenguin.net.Airtower
    {
        ...
        function setIsJumpToNewServerInProgress(_arg_2) {
            isJumpToNewServerInProgress = _arg_2;
        }
        ...
        function cloneListeners(_arg_5) {
            for (var _local_6 in sf_listener_container) {
                ...
                while (_local_3 < _local_4.length) {
                    ...
                    _arg_5.addListener(_local_6, _local_2.func, _local_2.scope);
                    _local_3++;
                }
            }
        }
        function addListener(_arg_2, _arg_5, _arg_6) {
            if ((_arg_2 == undefined) || (_arg_5 == undefined)) {
                shell.$e((("addListner() -> You must pass a valid listener type and function! type: " + _arg_2) + " func: ") + _arg_5);
                return(false);
            }
            var _local_3 = getListenersArray(_arg_2);
            var _local_4 = getListenerIndex(_local_3, _arg_5);
            if (_local_4 == -1) {
                shell.$d(("[airtower] Successfully added listener to: \"" + _arg_2) + "\"");
                _local_3.push({func:_arg_5, scope:_arg_6});
                return(true);
            }
            shell.$d(("[airtower] Failed to add listener. Tried to add a duplicate listener to: \"" + _arg_2) + "\"");
            return(false);
        }
        function removeListener(_arg_4, _arg_5) {
            var _local_2 = getListenersArray(_arg_4);
            var _local_3 = getListenerIndex(_local_2, _arg_5);
            if (_local_3 != -1) {
                shell.$d(("[airtower] Successfully removed listener from: \"" + _arg_4) + "\"");
                _local_2.splice(_local_3, 1);
                return(true);
            }
            shell.$d(("[airtower] Failed to remove listener which did not exist from: \"" + _arg_4) + "\"");
            return(false);
        }
        function getListenerIndex(_arg_3, _arg_4) {
            var _local_2 = _arg_3.length;
            var _local_1 = 0;
            while (_local_1 < _local_2) {
                if (_arg_3[_local_1].func == _arg_4) {
                    return(_local_1);
                }
                _local_1++;
            }
        ...
        function updateListeners(_arg_6, _arg_4) {
            var _local_3 = getListenersArray(_arg_6);
            var _local_5 = _local_3.length;
            if (_local_5 < 1) {
                shell.$d(("[airtower] No listeners currently attached to: \"" + _arg_6) + "\"");
                return(false);
            }
            if (_local_5 == 1) {
                (_local_3[0].scope ? (_local_3[0].func.call(_local_3[0].scope, _arg_4)) : (_local_3[0].func(_arg_4)));
                return(true);
            }
            var _local_2 = 0;
            while (_local_2 < _local_5) {
                (_local_3[_local_2].scope ? (_local_3[_local_2].func.call(_local_3[_local_2].scope, _arg_4)) : (_local_3[_local_2].func(_arg_4)));
                _local_2++;
            }
            return(true);
        }
        function getListenersArray(_arg_2) {
            if (sf_listener_container[_arg_2] == undefined) {
                sf_listener_container[_arg_2] = new Array();
            }
            return(sf_listener_container[_arg_2]);
        }
        function onAirtowerResponse(_arg_3, type, _arg_4) {
            if ((!_isBlocked) || (_arg_4 == true)) {
                var _local_2 = String(_arg_3[0]);
                if (com.clubpenguin.net.BridgeFilter.messageIsSentToAS3(this, _local_2)) {
                    shell.sendAirtowerEvent(_arg_3.join(SERVER_MESSAGE_DELIMITER) + SERVER_MESSAGE_DELIMITER);
                }
                ...
                switch (_local_2) {
                    ...
                    case "sz" : 
                    case "uw" : 
                        sendAck(_local_2, _arg_3.slice());
                }
                updateListeners(_local_2, _arg_3);
            }
        }
        function sendAck(_arg_4, _arg_3) {
            _arg_3 = _arg_3.slice();
            _arg_3.unshift(_arg_4);
            _arg_3.unshift("time=" + String(new Date().getTime()));
            this.send(PLAY_EXT, "bi#ack", _arg_3, "str", _global.getCurrentShell().getCurrentServerRoomId());
        }
        function isBlocked() {
            return(_isBlocked);
        }
        ...
        function setConnection(_arg_2) {
            server = _arg_2 || (server);
        }
        function setUsername(_arg_2) {
            username = _arg_2;
        }
        function setPlayerId(_arg_2) {
            playerId = _arg_2;
        }
        function connectToLogin(_arg_2, _arg_3, _arg_4, _arg_5) {
            ...
            com.clubpenguin.util.Log.debug("in_username: " + _arg_2);
            com.clubpenguin.util.Log.debug("in_pass: " + _arg_3);
            com.clubpenguin.util.Log.debug("login_response: " + _arg_4);
            com.clubpenguin.util.Log.debug("doAutoLogin: " + _arg_5);
        ...
        function handleLoginConnection(_arg_2) {
            if (_arg_2) {
                server.getRandomKey();
            } else {
                server.disconnect();
        ...
        function handleLoginRandomKey(_arg_2) {
            rand_key = _arg_2;
            login();
        }
        function login() {
        ...
        function handleOnLogin(_arg_2) {
            ...
            server.disconnect();
            com.clubpenguin.util.Log.debug("handleOnLogin: " + _arg_2);
            var _local_5 = _arg_2[1].split("|");
            loginObject = {};
            loginObject.loginDataRaw = _arg_2[1];
            ...
            loginObject.friendsLoginKey = _arg_2[3];
            loginObject.confirmationHash = _arg_2[2];
            loginObject.emailAddress = _arg_2[5];
            if (_arg_2[6] != null) {
                var _local_3 = _arg_2[6].split("|");
            ...
            shell.updateWorldPopulations(_arg_2[4]);
            var _local_9 = new Object();
            _local_9.type = 15;
            _local_9.playerSWID = loginObject.swid;
            _local_9.playerID = loginObject.playerID;
            shell.sendPlaydomTracking(_local_9);
            on_login_response(true);
            is_logged_in = true;
            com.clubpenguin.login.LoginFloodManager.clearFloodControl();
            var _local_10 = shell.getLanguageBitmask();
            com.clubpenguin.util.Log.debug("\t-loginObject.languageRejected: " + loginObject.languageRejected);
            com.clubpenguin.util.Log.debug("\t-lang: " + _local_10);
            com.clubpenguin.util.Log.debug((" -loginObject.languageRejected & lang: " + loginObject.languageRejected) + _local_10);
            if (loginObject.languageRejected & _local_10) {
        ...
        function handleExpiredPreactivationLogin(_arg_5) {
            this.removeListener(HANDLE_LOGIN, handleOnLogin);
            this.removeListener(HANDLE_LOGIN_EXPIRED, handleExpiredPreactivationLogin, this);
            ...
            _local_2.graceRemaining = _arg_5[1];
            _local_2.confirmationHash = _arg_5[2];
            _local_2.loginDataRaw = _arg_5[3];
            _local_2.emailAddress = _arg_5[4];
        ...
        function connectToRedemption(_arg_4, _arg_3, _arg_2) {
            isRedemption = true;
            on_world_response = _arg_2;
            ...
            server.connect(_arg_4, _arg_3);
        }
        function connectToWorld(_arg_4, _arg_3, _arg_2, loginKey) {
            isRedemption = false;
            on_world_response = _arg_2;
            server.onConnection = com.clubpenguin.util.Delegate.create(this, handleWorldConnection);
            server.onRandomKey = com.clubpenguin.util.Delegate.create(this, handleWorldRandomKey);
        ...
        function handleWorldConnection(_arg_2) {
            if (_arg_2) {
                server.getRandomKey();
            } else {
        ...
        function handleWorldRandomKey(_arg_2) {
            rand_key = _arg_2;
            worldLogin();
        }
        function worldLogin() {
        ...
        function handleJoinRedemption(_arg_2) {
            ...
            if (_arg_2[2] != "") {
                _local_6 = Boolean(Number(_arg_2[2]));
            }
        ...
        function handleJoinWorld(_arg_2) {
            com.clubpenguin.util.Log.debug(newline);
            com.clubpenguin.util.Log.debug("*********************************************");
            com.clubpenguin.util.Log.debug("handleJoinWorld");
            for (var _local_3 in _arg_2) {
                com.clubpenguin.util.Log.debug((("prop: " + _local_3) + "  ") + _arg_2[_local_3]);
            }
            com.clubpenguin.util.Log.debug("*********************************************");
            this.removeListener(JOIN_WORLD, handleJoinWorld);
            var _local_6 = Boolean(Number(_arg_2[1]));
            var _local_4 = Boolean(Number(_arg_2[2]));
            var _local_5 = Boolean(Number(_arg_2[4]));
            shell.playerModel.setUpModeratorPenguin(Number(_arg_2[3]));
        ...
        function handleGetActionStatus(_arg_3) {
            com.clubpenguin.util.Log.debug("handleGetActionStatus: " + _arg_3);
            this.removeListener(GET_ACTION_STATUS, handleGetActionStatus);
            var _local_4 = new Array(_arg_3[1], _arg_3[2], _arg_3[3]);
            shell.playerModel.initActionStatus(_local_4);
            _global.getCurrentEngine().localPlayerSpecialActionIndicators();
        }
        function handleGetActiveFeatures(_arg_2) {
            this.removeListener(GET_ACTIVE_FEATURES, handleGetActiveFeatures);
            shell.activeFeaturesArray = _arg_2.slice(1);
            shell.updateListeners(shell.UPDATE_ACTIVE_FEATURES);
        }
        function send(_arg_5, _arg_3, _arg_2, _arg_6, _arg_4) {
            com.clubpenguin.util.Log.debug("@@@@@ Airtower.send()  this=" + this);
            server.sendXtMessage(_arg_5, _arg_3, _arg_2, _arg_6, _arg_4);
        }
        function disconnect(_arg_2) {
            com.clubpenguin.util.Log.debug((("[airtower] disconnect() this = " + this) + "  disconnectFriends=") + _arg_2);
            if (_arg_2 == undefined) {
                _arg_2 = true;
            }
            if (server.connected()) {
        ...
        function hex_md5(_arg_1) {
            return(com.clubpenguin.crypto.MD5.hash(_arg_1));
        }
        function encryptPassword(_arg_2) {
            var _local_1 = com.clubpenguin.crypto.MD5.hash(_arg_2);
