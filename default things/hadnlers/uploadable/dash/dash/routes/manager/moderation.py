...
async def update_player(request):
    ...
    if type == 'id':
        ...
        return response.html(page)
    elif type == 'approval_en':
        await Penguin.update.values(approval_en=True,rejection_en=False).where(Penguin.id == player.id).gino.status()
        player = await Penguin.query.where(Penguin.id == player.id).gino.first()
        page = template.render(
            success_message='Updated approval en status.',
            error_message='',
            play_link=app.config.VANILLA_PLAY_LINK,
            player=player,
            penguin=data,
        ...
        return response.html(page)
    elif type == 'rejection_en':
        await Penguin.update.values(approval_en=False,rejection_en=True).where(Penguin.id == player.id).gino.status()
        player = await Penguin.query.where(Penguin.id == player.id).gino.first()
        page = template.render(
            success_message='Updated rejection en status.',
            error_message='',
            play_link=app.config.VANILLA_PLAY_LINK,
            player=player,
            penguin=data,
