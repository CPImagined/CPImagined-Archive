...
def stealth_mod_filter(player):
    def f(p):       #false = true
        return (not p.stealth_moderator or p.id == player.id) or player.moderator
    return f
...
class RoomMixin:
    ...
    async def remove_penguin(self, p):
        if not p.stealth_moderator:
            ...
            await self.send_xt('rp', p.id, f=lambda penguin: penguin.moderator)
    ...
    async def refresh(self, p):
        await p.send_xt('grs', self.id, await self.get_string(f=stealth_mod_filter(p)))
...
class Room(db.Model, RoomMixin):
    ...
    solo = db.Column(db.Boolean, nullable=False, server_default=db.text("false"))
    ...
    async def add_penguin(self, p):
        ...
        if self.game:
            ...
            await self.send_ap(p)

    async def send_ap(self, p):
        string = (await p.string) if self.solo else (await self.get_string(f=stealth_mod_filter(p)))
        await p.send_xt('jr', self.id, string)
        if self.solo:
            return await p.send_xt('ap', await p.string)
        if not p.stealth_moderator:
