...
async def handle_quest_award(p, quest_id: int):
    ...
    for award in quest.items:
        await p.add_inventory(p.server.items[award.item_id], cost=0)
    for award in quest.furniture:
        await p.add_furniture(p.server.furniture[award.furniture_id], cost=0)
    for award in quest.pet:
        await p.add_puffle_item(p.server.puffle_items[award.puffle_item_id], cost=0)
