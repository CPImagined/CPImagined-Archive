...
from houdini.data.mail import SpecialPostcardCollection, PenguinSpecialPostcardCollection
from houdini.handlers import XMLPacket, Priority
...
async def postcards_load(server):
    ...
    server.special_postcards = await SpecialPostcardCollection.get_collection()
    server.logger.info(f'Loaded {len(server.special_postcards)} special postcards')
...
async def handle_get_mail(p):
    ...
    async with p.server.db.transaction():
        async for postcard in mail_query.gino.iterate():
            ...
            offset = datetime.timedelta(seconds=(postcard.send_date - pst_send_date).total_seconds())
...
@handlers.allow_once
async def handle_special_postcards(p):
    # We don't need to store this as we're only doing this once on login
    received_special_mail = await PenguinSpecialPostcardCollection.get_collection(p.id)

    pst = pytz.timezone(p.server.config.timezone)
    
    for sp in list(p.server.special_postcards.values()):
        # Has this penguin already received the mail?
        if sp.id in received_special_mail:
            continue
    
        sp_start_date = sp.start_date.astimezone(pst)
        sp_end_date = sp.end_date.astimezone(pst)
        now = datetime.datetime.now(tz=pst)
        
        # Is the mail being sent right now?
        if sp_start_date <= now <= sp_end_date:
            await received_special_mail.insert(special_postcard_id=sp.id)
            await p.add_inbox(p.server.postcards[sp.postcard_id], sender_id=sp.sender_id)