...
@handlers.handler(XTPacket('o', 'mpl'))
async def handle_move_player(p, player_id: int, x: int, y: int):
    if p.moderator:
     await p.room.send_xt('sp', player_id, x, y)
...
async def moderator_kick(p, penguin_id):
    if penguin_id in p.server.penguins_by_id:
        ...
        if not player.moderator:
            ...
            await player.send_xt('moderatormessage', 3)
            await player.close()
