...
async def setup_membership(p):
    all_membership_history= await PenguinMembership.query.where(PenguinMembership.penguin_id == p.id).gino.all()  

    # not p.server.config.expire_membership
    if not all_membership_history:
        p.is_member = True
...
@handlers.handler(XTPacket('u', 'jbf'), client=ClientType.Vanilla)
async def handle_find_player_jump(p, player_id: int):
    if player_id in p.server.penguins_by_id:
        player = p.server.penguins_by_id[player_id]
        room_id = player.room.id
        room_type = 'backyard' if player.room.backyard else 'igloo' if player.room.igloo else 'invalid'
        ...
        room_id, room_type, room_owner = -1, 'invalid', -1
    await p.send_xt('jbf', room_id, room_type, room_owner)
...
@handlers.handler(XTPacket('u', 'tp'))
async def handle_player_teleport(p, x: int, y: int):
    p.x, p.y = x, y
    p.frame = 1
    p.toy = None
    await p.room.send_xt('tp', p.id, x, y)
