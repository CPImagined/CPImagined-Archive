//Created by Action Script Viewer - https://www.buraks.com/asv
    class com.clubpenguin.engine.projectiles.SnowballManager
    {
        ...
        function throwBall(playerId, target_x, target_y, snowballVO) {
            ...
            var room = _ENGINE.getRoomMovieClip();
			var snolaf = _global.getCurrentSnolaf();
			snolaf.testSound();
			var snowballOverride = room.snowballOverride;
            var _local_15 = room.customPenguinSize;
            var _local_22 = _SHELL.getRoomObject().room_id;
            var _local_5;
            var _local_11;
            var _local_18 = _SHELL.getPlayerObjectById(playerId);
			var _local_19 = _SHELL.getPlayerObjectById(playerId).avatarVO.avatar_id;
            if ((snowballVO == undefined) || (snowballVO == null)) {
                snowballVO = _local_18.avatarVO.snowball;
            }
            if (_local_19 == 0 && snowballOverride) {
            ...
            /*} else {
                var _local_14 = snolaf.attachMovie(snowballVO.type.symbolName, "testSnowball", 1);
                if (_local_14 == undefined) {
                    snowballVO = new com.clubpenguin.engine.projectiles.vo.SnowballVO(com.clubpenguin.engine.projectiles.SnowballEnum.SNOW_BALL_NORMAL, -100, 20);
                } else {
                    _local_14.removeMovieClip();
                };
				var testSnowball2 = snolaf.attachMovie(snowballVO.type.symbolName, "testSnowball", 1);
                if (testSnowball2 == undefined) {
                    snowballVO = new com.clubpenguin.engine.projectiles.vo.SnowballVO("ball", -100, 20);
                } else {
                    testSnowball2.removeMovieClip();
                };*/
            }
            ...
            if (_local_3.is_ready && (!_local_3.is_moving)) {
                var _local_17 = _ENGINE.findAngle(_local_3._x, _local_3._y, target_x, target_y);
                var _local_16 = _ENGINE.findDirectionQuarters(_local_17);
                _ENGINE.updatePlayerFrame(playerId, _SHELL.DANCE_FRAME + _local_16);
                var _local_9;
                ...
                if (room[_local_11] != undefined) {
                    room[_local_11].removeMovieClip();
                }
                _local_5 = room.attachMovie(snolaf[snowballVO.type.symbolName], _local_11, Number(1000200 + throwItemCounter), {_x:_local_2.start_x, _y:_local_2.start_y});
                if (_local_5 == undefined) {
                    _local_5 = room.attachMovie("ball", _local_11, Number(1000200 + throwItemCounter), {_x:_local_2.start_x, _y:_local_2.start_y});
                }
                ...
                if (snowballVO.type.hasTail) {
                    _local_5.art._rotation = com.clubpenguin["math"].MathHelper.getAngleBetweenCoordinates(_local_9.x, _local_9.y, target_x, target_y);
                }
                if (snowballVO.type.playAbovePlayer && (_local_5.getDepth() < _local_3.getDepth())) {
