...
StartParty = datetime(2021, 5, 28, 00, 00, 0)#date[year month day hour minute sec] for start party
EndParty = datetime(2021, 6, 28, 00, 00, 0)#date[year month day hour minute sec] for end party

CookieHandlerName = "fair"
PartyCacheCurrent = "fair1year_version2" #if you have a repeated party, it is advisable to make a different name every time
...
async def givePartyCoins(p, coins: int):
    if random.randrange(1000) in range(0,10):
        party_coins = p.get_custom_attribute('party_coins')
        if(party_coins != None):
            await p.set_custom_attribute('party_coins', str(int(party_coins)+coins))
        else:
            await p.set_custom_attribute('party_coins', str(coins))
        return await p.send_xt('showContent', 'dialog')
    return None
...
async def handle_fixed_bounce(p, score: int):#fixed get ticket for game super hero bounce
    cookie = await p.server.redis.hget(PartyCacheCurrent, p.id)
    cookie = ujson.loads(cookie)
    
    if p.room.id == 940:
        cookie['tickets'] += score
        await givePartyCoins(p, 1)
...
async def handle_party_endgame(p, tickets: int):
    ...
    cookie['tickets'] += tickets
    await givePartyCoins(p, 1)
    
    await p.server.redis.hset(PartyCacheCurrent, p.id, ujson.dumps(cookie))
    await p.update(coins=p.coins).apply()
...
@handlers.handler(XTPacket(CookieHandlerName, 'fsilverjr'))
async def handle_party_silverjr(p, room: Room, x, y):
    cookie = await p.server.redis.hget(PartyCacheCurrent, p.id)
    cookie = ujson.loads(cookie)
    
    if(cookie['silverTicket'] > 0):
        await p.join_room(room)
        cookie['silverTicket'] -= 1
...
async def handle_party_useticket(p, item_id: int):
    cookie = await p.server.redis.hget(PartyCacheCurrent, p.id)
    cookie = ujson.loads(cookie) 
    if int(cookie['tickets']) <= 0 or int(cookie['tickets']) < ItemCost[item_id]: return False
    ...
    cookie['tickets'] -= ItemCost[item_id]
    await p.server.redis.hset(PartyCacheCurrent, p.id, ujson.dumps(cookie))
...
async def handle_get_partysettings(p):
    if (datetime.now() < EndParty):#check enabled party, if the current datatime has exceeded the end of the party, then do not start
        ...
        if cookie is None:
            ...
            cookie = cookie.decode('utf-8')
        await p.send_packet('partycookie', cookie)
        await p.send_partypacket('partyservice', '{"partySettings":{"unlockDayIndex":' + str(TotalDayParty()) + ',"numOfDaysInParty":16},"partyStartDate":"' + str(StartParty) + '","partyEndDate":"' + str(EndParty) + '"}')
