...
CookieHandlerId = 20160901

StartParty = datetime(2022, 1, 17, 00, 0, 0)#date[year month day hour minute sec] for start party
EndParty = datetime(2022, 11, 20, 00, 0, 0)#date[year month day hour minute sec] for end party

CookieHandlerName = "party"
PartyCacheCurrent = "halloween22"
...
async def handle_get_partysettings(p):
    if (datetime.now() < EndParty):#check enabled party, if the current datatime has exceeded the end of the party, then do not start
        ...
        if cookie is None:
            ...
            cookie = cookie.decode('utf-8')
        await p.send_xt('partycookie', cookie)
        await p.send_xt('partyservice', '{"partySettings":{"unlockDayIndex":31,"numOfDaysInParty":31},"partyStartDate":"2015-10-21 00:00:00","partyEndDate":"2015-11-05 00:00:00"}')
