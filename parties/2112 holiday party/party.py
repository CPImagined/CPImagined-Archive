...
StartParty = datetime(2022, 7, 5, 00, 0, 0)#date[year month day hour minute sec] for start party
EndParty = datetime(2023, 1, 10, 00, 0, 0)#date[year month day hour minute sec] for end party

CookieHandlerName = "party"
PartyCacheCurrent = "holidayparty22"
...
async def handle_party_cfc_donate(p, coins: int):
    if p.coins >= coins and 100 <= coins <= 10000:
        ...
        if(donate_player > 0):
            #await p.add_inventory(p.server.items[7320], cost=0)
...
async def handle_get_partysettings(p):
    if (datetime.now() < EndParty):#check enabled party, if the current datatime has exceeded the end of the party, then do not start
        ...
        await p.send_partypacket('partyservice', '{"partySettings":{"unlockDayIndex":' + str(TotalDayParty()) + ',"numOfDaysInParty":30},"partyStartDate":"2015-10-21 00:00:00","partyEndDate":"2015-11-05 00:00:00"}')
