...
package com.clubpenguin.tools.localtext.component.controlpanel
{
    ...
    public class LTCjsfl 
    {
        ...
        private static const MIN_URL_LENGTH:uint = 3;


        public static function jsonUrlExists():Boolean
        {
            var _local_1:String = getJsonUrl();
            return (((!(_local_1 == null)) && (_local_1.length > MIN_URL_LENGTH)) && (!(_local_1 == "0")));
        }

        public static function setJsonUrl(_arg_1:String):void
        {
            setDocumentProperty("jsonUrl", _arg_1);
        }

        public static function getJsonUrl():String
        {
            return (getDocumentPropertyValue("jsonUrl"));
        }

        public static function runExternalSwf(_arg_1:String, _arg_2:Boolean=true):void
        {
            var _local_3:String = ((_arg_2) ? "'" : "");
            MMExecute((((("fl.openScript(" + _local_3) + _arg_1) + _local_3) + ");"));
        }

        public static function setDocumentProperty(_arg_1:String, _arg_2:String, _arg_3:String="string"):void
        {
            MMExecute((((((("fl.getDocumentDOM().addDataToDocument('" + _arg_1) + "', '") + _arg_3) + "', '") + _arg_2) + "');"));
        }

        public static function getDocumentPropertyValue(_arg_1:String):String
        {
            return (MMExecute((("fl.getDocumentDOM().getDataFromDocument('" + _arg_1) + "');")));
        }
        ...
        {
            var _local_1:* = (((((((((((("function getParamNames() {" + "var selection = fl.getDocumentDOM().selection[0];") + "if (selection != undefined) {") + "var paramNames = new Array();") + "for (var i = 0; i < selection.parameters.length; i++) {") + "\tparamNames[i] = selection.parameters[i].name;") + "}") + "return paramNames;") + "} else {") + "\tfl.outputPanel.trace('ERROR: Valid LocalTextComponent Live Preview is not selected!');") + "}") + "return [];") + "} getParamNames();");
        ...
        public static function getAssignmentJSFLForParamIndex(_arg_1:uint, _arg_2:String, _arg_3:*, _arg_4:Boolean=false):String
        {
            ...
            var _local_6:* = ((((((getValueJSFLForParamIndex(_arg_1, _arg_2) + " = ") + _local_5) + _arg_3) + _local_5) + ";") + "fl.getDocumentDOM().livePreview = true;");
            return (_local_6);
        }
        ...
        {
            var _local_2:* = ((((((((("function getListItemNames() {" + "var listItemParameters = fl.getDocumentDOM().selection[0].parameters[") + _arg_1) + "].value;") + "var listItemNames = new Array();") + "for (var i = 0; i < listItemParameters.length; i++) {") + "\tlistItemNames[i] = listItemParameters[i].value;") + "}") + "return listItemNames;") + "} getListItemNames();");
        ...
        {
            var _local_1:* = (MMExecute("fl.getDocumentDOM().selection[0].parameters[0].name;") == "");
            var _local_2:String = MMExecute((("fl.getDocumentDOM().getDataFromDocument('" + CS6_BUG_FL_DOC_LTC_PARAM_NAME) + "');"));
            var _local_3:* = (!(parseInt(_local_2) == 0));
            if (!_local_1)
            {
        ...
        public static function flTrace(_arg_1:String):void
        {
            MMExecute((("fl.trace('" + _arg_1) + "');"));
        }

        public static function getPathToPublishedSWF():String
        {
            var _local_1:* = (((((((((((("function getPathToPublishedSWF() {\n" + "\tvar profile = new XML(fl.getDocumentDOM().exportPublishProfileString());\n") + "\tvar publishPath = profile.PublishFormatProperties.flashFileName;\n") + "\tvar pathPrefix = '';\n") + "\tif ((publishPath.indexOf('/') == -1 && publishPath.indexOf(\"\\\\\") == -1) || publishPath == undefined || \n") + "\t\t\tpublishPath.indexOf('..') > -1 || publishPath.indexOf('./') > -1 || publishPath.length == 0) {\n") + "\t\tvar split = fl.getDocumentDOM().pathURI.split('/');\n") + "\t\tsplit.length = split.length -1;\n") + "\t\tpathPrefix = split.join('/') + '/';\n") + "\t}\n") + "\treturn pathPrefix + publishPath;\n") + "}\n") + "getPathToPublishedSWF();\n");
            return (MMExecute(_local_1));
        }

        public static function publishSWF():void
        {
            MMExecute("fl.getDocumentDOM().publish()");
        }
