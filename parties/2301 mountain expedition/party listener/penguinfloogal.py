...
class Penguin(db.Model):
    ...
    married = db.Column(db.ForeignKey('penguin.id', ondelete='CASCADE', onupdate='CASCADE'))
    bff = db.Column(db.ForeignKey('penguin.id', ondelete='CASCADE', onupdate='CASCADE'))
...
class PartyQuest(db.Model):
    __tablename__ = 'penguin_party_quest'

    penguin_id = db.Column(db.ForeignKey('penguin.id', ondelete='CASCADE', onupdate='CASCADE'), nullable=False)
    quest_id = db.Column(db.Integer, nullable=False)

class PartyQuestCollection(AbstractDataCollection):
    __model__ = PartyQuest
    __indexby__ = 'penguin_id'
    __filterby__ = 'quest_id'

class FairTickets(db.Model):
    __tablename__ = 'penguin_fair_tickets'

    penguin_id = db.Column(db.ForeignKey('penguin.id', ondelete='CASCADE', onupdate='CASCADE'), nullable=False)
    tickets = db.Column(db.Integer)

class FairTicketsCollection(AbstractDataCollection):
    __model__ = FairTickets
    __indexby__ = 'penguin_id'
    __filterby__ = 'tickets'

class Outfits(db.Model):
    __tablename__ = 'penguin_outfits'

    penguin_id = db.Column(db.ForeignKey('penguin.id', ondelete='CASCADE', onupdate='CASCADE'), nullable=False)
    outfit_id = db.Column(db.Integer, primary_key=True, server_default=db.text("nextval('\"outfit_id_seq\"'::regclass)"))
    outfit_name = db.Column(db.String(16), nullable=False)
    color = db.Column(db.ForeignKey('item.id', ondelete='CASCADE', onupdate='CASCADE'))
    head = db.Column(db.ForeignKey('item.id', ondelete='CASCADE', onupdate='CASCADE'))
    face = db.Column(db.ForeignKey('item.id', ondelete='CASCADE', onupdate='CASCADE'))
    neck = db.Column(db.ForeignKey('item.id', ondelete='CASCADE', onupdate='CASCADE'))
...
class SavedOutfitCollection(AbstractDataCollection):
    __model__ = Outfits
    __indexby__ = 'outfit_name'
    __filterby__ = 'penguin_id'

class LastSpin(db.Model):
    __tablename__ = 'penguin_last_spin'

    penguin_id = db.Column(db.ForeignKey('penguin.id', ondelete='CASCADE', onupdate='CASCADE'), nullable=False)
    last_spin = db.Column(db.BigInteger)

class LastSpinCollection(AbstractDataCollection):
    __model__ = LastSpin
    __indexby__ = 'penguin_id'
    __filterby__ = 'last_spin'
