//Created by Action Script Viewer - https://www.buraks.com/asv
    class com.clubpenguin.quests.rainbowpuffle.RainbowPuffleQuest
    {
        ...
        function completePuffleQuestTask(taskNumber) {
            trace("RainbowPuffleQuest.completePuffleQuestTask enter");
            ...
            trace("RainbowPuffleQuest - PENGUIN TIME " + _shell.getPenguinStandardTime().getTime());
            trace("RainbowPuffleQuest - client calculated timeUntilNextTask " + _local_8);
            var _local_5 = 0;
            var _local_4 = 0;
            var _local_6 = 0;
            var _local_3 = 0;
        ...
        function onTaskCoinCollectedReceived(data) {
            trace("RainbowPuffleQuest.onTaskCoinCollectedReceived SERVER SAYS COIN COLLECTED");
            var _local_6 = _serverCookie.taskCollectables;
            trace("RainbowPuffleQuest.onTaskCoinCollectedReceived " + data);
            var _local_4 = data[1];
            var _local_5 = data[2];
            var _local_2 = data[3];
        ...
        function onTaskItemCollectedReceived(data) {
            trace("RainbowPuffleQuest.onTaskItemCollectedReceived SERVER SAYS ITEM COLLECTED");
            ...
            trace((((((("taskIndex " + _local_3) + " itemID ") + _local_2) + " is item in my inventory ") + _shell.isItemInMyInventory(_local_2)) + " has item been collected ") + _local_4[_local_3].item);
            if (((_local_2 > 0) && (!_shell.isItemInMyInventory(_local_2))) && (_local_4[_local_3].item != COLLECTED)) {
                trace("RainbowPuffleQuest.onTaskItemCollectedReceived adding item to inventory");
        ...
        function onTaskBonusCollectedReceived(data) {
            trace("RainbowPuffleQuest.onTaskItemCollectedReceived SERVER SAYS ITEM COLLECTED");
            var _local_7 = data[2];
            var _local_2 = PUFFLE_BONUS_ITEM_ID;
            ...
            if (((_local_2 > 0) && (!_shell.isItemInMyInventory(_local_2))) && (puffleQuestBonusState != COLLECTED)) {
                _local_4.push(_local_5);
                showItemCollectedPrompt(_local_2);
            }
            var _local_3 = data[3];
            showCoinsCollectedPrompt(_local_3 - _shell.getMyPlayerTotalCoins(), _local_3);
