...
StartParty = datetime(2023, 1, 15, 00, 0, 0)#date[year month day hour minute sec] for start party
EndParty = datetime(2023, 12, 5, 00, 0, 0)#date[year month day hour minute sec] for end party

CookieHandlerName = "party"
PartyCacheCurrent = "musicjam2023"
...
async def handle_get_partysettings(p):
    if (datetime.now() < EndParty):#check enabled party, if the current datatime has exceeded the end of the party, then do not start
        ...
        await p.send_partypacket('partyservice', '{"partySettings":{"unlockDayIndex":20,"numOfDaysInParty":20},"partyStartDate":"2015-10-21 00:00:00","partyEndDate":"2015-11-05 00:00:00"}')
