...
class WaddleOn(IPlugin):
    ...
    async def ready(self):
        ...
        await self.server.permissions.register('waddleon.postcard')
    ...
    async def change_avatar(self, p, avatar_id: int):
            p.avatar = avatar_id
            await p.send_xt('spts', p.id, avatar_id)
    ...
    async def event(self, p):
        try:
            await p.room.send_xt('liveevent', h.id)
        except KeyError:
            await p.send_xt('mm', 'something has gone catastrophically wrong', p.id)

    @commands.command('concert')
    @permissions.has_or_moderator('waddleon.global')
    async def event(self, p, concert):
        try:
    ...
    @permissions.has_or_moderator('waddleon.postcard')
    async def sendcarpost(self, p, postcard_id):
        try:
            for t in p.room.penguins_by_id.values():
                await t.add_inbox(p.server.postcards[postcard_id], sender_id=p)
                await p.send_xt('mm', f'Successfully sent postcard to all players in room', p.id)
        except KeyError:
            await p.send_xt('mm', 'something has gone catastrophically wrong', p.id)

    #@commands.command('postcard')
    #@permissions.has_or_moderator('waddleon.postcard')
    #async def sendcardpost(self, p, postcard_id):
    #    t = len(p.room.penguins_by_id)
    #    await t.add_inbox(p.server.postcards[postcard_id], sender_id=p)
    #    await p.send_xt('mm', f'successfully sent postcard to all players in room', p.id)
    ...
    async def addallcards(self, p):
        for a in self.server.cards.power_cards:
            await p.add_card(a)
