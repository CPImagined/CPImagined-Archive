dynamic class com.clubpenguin.net.Airtower
{
    ...
    {
        var __reg4 = "";
        var __reg3 = this.getServer().getSentCommandsBuffer();
        var __reg2 = 0;
        while (__reg2 < __reg3.length) 
        {
            __reg4 = __reg4 + __reg3[__reg2];
            if (__reg2 != __reg3.length - 1) 
            {
                __reg4 = __reg4 + "\n";
            }
            ++__reg2;
    ...
    {
        var __reg2 = this.getListenersArray(type);
        var __reg3 = this.getListenerIndex(__reg2, func);
        if (__reg3 != -1) 
        {
    ...
    function onAirtowerResponse(eventData, type, overrideBlock)
    {
        if (!this._isBlocked || overrideBlock == true) 
        {
            ...
            {
                trace("Airtower.onAirTowerResponse messageIsSentToAS3");
                this.shell.sendAirtowerEvent(eventData.join(this.SERVER_MESSAGE_DELIMITER) + this.SERVER_MESSAGE_DELIMITER);
            }
            else 
            {
                trace("Airtower.onAirTowerResponse NOT messageIsSentToAS3");
            }
    ...
    function handleOnLogin(serverResponse)
    {
        ...
        {
            var __reg3 = serverResponse[6].split("|");
            this.loginObject.remaining_hours = __reg3[0];
            this.loginObject.trialMax = __reg3[1];
            this.loginObject.max_grace_hours = __reg3[2];
            var __reg6 = new Object();
            __reg6.modalBackgroundEnbaled = true;
            __reg6.blockPuffleNotifications = true;
            __reg6.hideLoadingDialog = true;
            var __reg4 = new Object();
            __reg4.state = this.FRAME_LABEL_WELCOME;
            __reg4.trialRemaining = __reg3[0];
            __reg4.trialMax = __reg3[1];
            __reg4.graceMax = __reg3[2];
            __reg4.confirmationHash = this.loginObject.confirmationHash;
            __reg4.loginDataRaw = this.loginObject.loginDataRaw;
            this.shell.sendOpenAS3Module("preactivation", __reg4, __reg6);
            flash.external.ExternalInterface.call("showActivationBanner", __reg3[0]);
        }
        for (var __reg7 in serverResponse) 
        {
            trace("\t-serverResponse[" + __reg7 + "]: " + serverResponse[__reg7]);
        }
        for (var __reg8 in this.loginObject) 
        {
            com.clubpenguin.util.Log.debug("loginObject." + __reg8 + ":" + this.loginObject[__reg8]);
        }
        ...
        __reg9.type = 15;
        __reg9.playerSWID = this.loginObject.swid;
        __reg9.playerID = this.loginObject.playerID;
        this.shell.sendPlaydomTracking(__reg9);
    ...
    function handleExpiredPreactivationLogin(serverResponse)
    {
        ...
        var __reg4 = this.shell.getLanguageBitmask();
        trace("\t-loginObject.languageRejected: " + this.loginObject.languageRejected);
        trace("\t-lang: " + __reg4);
        trace(" -loginObject.languageRejected & lang: " + this.loginObject.languageRejected & __reg4);
        if (this.loginObject.languageRejected & __reg4) 
        {
    ...
    function worldLogin()
    {
        ...
        trace("Airtower - loginDataRaw " + this.shell.getMyPlayerObject().loginDataRaw);
    ...
    function handleJoinWorld(obj)
    {
        ...
        var __reg6 = Boolean(Number(obj[1]));
        var __reg4 = Boolean(Number(obj[2]));
        var __reg5 = Boolean(Number(obj[4]));
        this.shell.playerModel.setUpModeratorPenguin(Number(obj[3]));
        this.on_world_response(true, __reg6, __reg4, __reg5);
        this.on_world_response = undefined;
        this.isJump = false;
