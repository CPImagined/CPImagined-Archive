dynamic class com.clubpenguin.net.Connection extends XMLSocket implements com.clubpenguin.net.IConnection
{
    ...
    function message2Object(xmlNodes, parentObj)
    {
        ...
        for (;;) 
        {
            ...
            var __reg5 = __reg4.nodeName;
            var __reg6 = __reg4.nodeValue;
            ...
            {
                parentObj[__reg5] = new Object();
                __reg3 = parentObj[__reg5];
            }
            ...
            {
                __reg6 = __reg4.firstChild.nodeValue;
                if (!isNaN(__reg6) && __reg4.nodeName != "txt" && __reg4.nodeName != "var") 
                {
                    __reg6 = Number(__reg6);
                }
                __reg3.value = __reg6;
            }
            ++__reg8;
    ...
    function makeHeader(headerObj)
    {
        ...
        {
            __reg2 = __reg2 + (" " + __reg3 + "=\'" + headerObj[__reg3] + "\'");
        }
        __reg2 = __reg2 + ">";
        return __reg2;
    }
    ...
    function sendXtMessage(xtName, cmdName, paramObj, type, roomId)
    {
        ...
        var __reg3 = cmdName.split("#")[1];
        var __reg2 = 0;
        while (__reg2 < this.disabledCommands.length) 
        {
            if (this.disabledCommands[__reg2] == __reg3) 
            {
                trace("sendXTMessage command: " + __reg3 + " is disabled.");
                return undefined;
            }
            ++__reg2;
        }
        ...
        if (type == "str") 
        {
            ...
            __reg5 = this.serverMessageDelimiter + "xt" + this.serverMessageDelimiter + xtName + this.serverMessageDelimiter + cmdName + this.serverMessageDelimiter + roomId + this.serverMessageDelimiter;
            __reg2 = 0;
            while (__reg2 < paramObj.length) 
            {
                __reg5 = __reg5 + (paramObj[__reg2].toString() + this.serverMessageDelimiter);
                ++__reg2;
            }
            this.sendString(__reg5);
        }
    }
    ...
    function connectionEstablished(ok)
    {
        ...
        if (ok) 
        {
            ...
            this.send(__reg4, "verChk", 0, __reg3);
            return;
        }
    ...
    function strReceived(message)
    {
        ...
        var __reg6 = __reg4[0];
        this._responded.dispatch(__reg3, __reg7);
        this.messageHandlers[__reg6].handleMessage(__reg4.splice(1, __reg4.length - 1), this, "str");
    }
    ...
    function handleExtensionMessages(dataObj, scope, type)
    {
        ...
        if (type == "xml") 
        {
            var __reg6 = dataObj.attributes.action;
            var __reg7 = dataObj.attributes.r;
            if (__reg6 == "xtRes") 
            {
                var __reg4 = dataObj.value;
                var __reg5 = scope.os.deserialize(__reg4);
    ...
    function handleSysMessages(xmlObj, scope)
    {
        ...
        if (__reg1 == "rndK") 
        {
            var __reg2 = xmlObj.k.value;
            scope.onRandomKey(__reg2);
        }
    }
