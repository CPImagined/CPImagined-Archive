dynamic class com.clubpenguin.world.rooms.BaseRoom
{
    ...
    function navigationButtonClick(navigationButtons, whichButton)
    {
        ...
        if (this._ENGINE.isMovementEnabled()) 
        {
            ...
            var __reg2 = Math.round(navigationButtons[whichButton].navigationX);
            var __reg3 = Math.round(navigationButtons[whichButton].navigationY);
            this._ENGINE.sendPlayerMove(__reg2, __reg3);
            com.clubpenguin.util.Log.info("setupNavigationButtons.navigationButtonClick: _ENGINE.sendPlayerMove(" + __reg2 + ", " + __reg3 + ")");
        }
    }
    ...
    function disablePuffleActivities()
    {
        ...
        __reg3.playerId = __reg2.player_id;
        this._SHELL.updateListeners(this._SHELL.DISABLE_PUFFLE_TREASURE_HUNTING, __reg2.player_id);
        this._SHELL.updateListeners(this._SHELL.DISABLE_PUFFLE_DIG_SEARCH_EMOTE, __reg3);
    }
    ...
    {
        var __reg2 = Math.round(tableObjects[whichClip].tableClip._x);
        var __reg3 = Math.round(tableObjects[whichClip].tableClip._y);
        this._ENGINE.sendPlayerMove(__reg2, __reg3);
        com.clubpenguin.util.Log.info("setupTableGame.clickTable: _ENGINE.sendPlayerMove(" + __reg2 + ", " + __reg3 + ")");
    }
