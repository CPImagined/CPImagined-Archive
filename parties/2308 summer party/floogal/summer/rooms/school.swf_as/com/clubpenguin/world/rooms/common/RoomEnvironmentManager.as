dynamic class com.clubpenguin.world.rooms.common.RoomEnvironmentManager
{
    ...
    function checkTriggers(player_obj)
    {
        ...
        for (;;) 
        {
            if (__reg3 >= this._environmentMovieClips.length) 
            {
                return undefined;
            }
            var __reg2 = this._ENGINE.puffleAvatarController.getPuffleClip(player_obj.attachedPuffle.id);
            if (__reg2 && this._environmentMovieClips[__reg3].hitTest(__reg2._x, __reg2._y, true)) 
            {
                var __reg4 = this._roomBehaviorFactory.getRoomBehavior(this._environmentMovieClips[__reg3].environmentType);
