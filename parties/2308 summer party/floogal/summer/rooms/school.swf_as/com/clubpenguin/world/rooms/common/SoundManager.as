dynamic class com.clubpenguin.world.rooms.common.SoundManager
{
    ...
    function destroy()
    {
        for (var __reg4 in this._soundLoopsMap) 
        {
            var __reg3 = this._soundLoopsMap[__reg4];
            this.stopSoundLoop(__reg3.loopId);
        }
        for (var __reg5 in this._delayedSoundsMap) 
        {
            var __reg2 = this._delayedSoundsMap[__reg5];
            this.cancelDelayedSound(__reg2.delayedSoundId);
        }
        this._frameCounter.removeMovieClip();
    ...
    function playSound(symbolId, layerId, volume, tag, callback)
    {
        ...
        {
            var __reg8 = this._registeredSymbolNamesMap[symbolId];
            if (__reg8 == null || this._sfxHolder == null) 
            {
                return -1;
            }
            var __reg7 = this._sfxHolder.getNextHighestDepth();
            var __reg6 = this._sfxHolder.createEmptyMovieClip("sfx_" + symbolId + "_" + __reg7, __reg7);
            var __reg3 = new Sound(__reg6);
            __reg3.attachSound(__reg8);
            if (__reg3.duration != undefined) 
            {
                var __reg2 = new Object();
                var __reg5 = this.getSoundId();
                __reg2.soundId = __reg5;
                __reg2.tag = tag;
                __reg2.sfx = __reg3;
                __reg2.layerId = layerId;
                __reg2.targetHolder = __reg6;
                __reg2.callback = callback;
                __reg2.symbolId = symbolId;
                __reg2.layer = __reg4;
                this._activeSoundsMap[__reg5] = __reg2;
                ++__reg4.numSoundsPlaying;
                __reg3.onSoundComplete = com.clubpenguin.util.Delegate.create(this, this.onSoundComplete, __reg3, __reg4, __reg6, tag, callback);
    ...
    function stopSoundsForTag(tag)
    {
        ...
        {
            var __reg2 = this._activeSoundsMap[__reg6];
            if (__reg2.tag == tag) 
            {
                __reg2.sfx.stop();
                this.onSoundComplete(__reg2.sfx, __reg2.layer, __reg2.targetHolder, __reg2.callback);
                delete this._activeSoundsMap[__reg2.soundId];
            }
        }
        for (var __reg5 in this._soundLoopsMap) 
        {
            var __reg3 = this._soundLoopsMap[__reg5];
            if (__reg3.tag == tag) 
            {
                this.stopSoundLoop(__reg3.loopId);
            }
        }
        for (var __reg7 in this._delayedSoundsMap) 
        {
            __reg2 = this._delayedSoundsMap[__reg7];
            if (__reg2.tag == tag) 
            {
                this.cancelDelayedSound(__reg2.delayedSoundId);
            }
