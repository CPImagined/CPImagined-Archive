...
EndParty = datetime(2024, 3, 23, 00, 00, 0)#date[year month day hour minute sec] for end party

CookieHandlerName = "fair"
PartyCacheCurrent = "hallo23" #if you have a repeated party, it is advisable to make a different name every time
...
@handlers.handler(XTPacket('anniv', 'rentaride'))
async def handle_rent_a_ride(p):
    await p.update(coins=p.coins-100).apply()
    p.avatar = 2001
    await p.room.send_xt('spts', p.id, 2001)
