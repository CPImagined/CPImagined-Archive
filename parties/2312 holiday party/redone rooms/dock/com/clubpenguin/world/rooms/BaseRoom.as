//Created by Action Script Viewer - https://www.buraks.com/asv
    ...
    {
        static var _current, lastRoomTrainManager;
        var _stage, _SHELL, _ENGINE, _INTERFACE, _destroyFunc, _roomEnvironmentManager, _trainManager;
        function BaseRoom (stageReference) {
            ...
            _trainManager = new com.clubpenguin.world.rooms2013.cfc.trains.TrainManager();
            if (com.clubpenguin.world.rooms2013.cfc.CFCParty.showRunAwayTrainOnJoinRoom) {
                com.clubpenguin.world.rooms2013.cfc.CFCParty.showRunAwayTrainOnJoinRoom = false;
                _INTERFACE.showContent("w.p2013.cfc.runawaytrain");
            }
        }
        static function get current() {
            return(_current);
        }
        function get trainManager() {
            return(_trainManager);
        }
        function onDestroy() {
            _SHELL.removeListener(_SHELL.ROOM_DESTROYED, _destroyFunc);
            _roomEnvironmentManager.destroy();
            _trainManager.destroy();
            lastRoomTrainManager = _trainManager;
            _current = null;
        }
        function getStage() {
            return(_stage);
