...
class DinoDigLogic(ITable):
    ...
    def get_puffle_egg(self):
        """
            ...
            PUFFLE EGG IDS: 
                ...
                15 => yellow
        """
        return random.choices([400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 417, 418, 419, 10, 11, 12, 13, 14, 15], weights=[13, 1, 3, 7, 7, 1, 13, 3, 3, 13, 7, 1, 3, 13, 7, 1, 1, 3, 7, 13, 13, 1, 1, 7, 13, 7])[0]
    ...
    def generate_treasure(self, row, column):
        ...
        name, value = random.choices(treasure_type, weights=[60, 25, 15][:len(treasure_type)])[0]
        if value == 1:
            self.coins_hidden += 1
        if value > 1:
