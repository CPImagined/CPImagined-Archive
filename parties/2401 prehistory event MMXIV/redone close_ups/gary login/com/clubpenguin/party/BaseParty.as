//Created by Action Script Viewer - https://www.buraks.com/asv
    class com.clubpenguin.party.BaseParty
    {
        ...
        static function configurePurchaseableItem(itemMC, itemID, itemType, member, prompt, purchasedCallback) {
            ...
            itemMC[CLAIM_BUTTON_INSTANCE_NAME].onRelease = com.clubpenguin.util.Delegate.create(_this, checkPurchaseOfItem, itemMC, itemID, itemType, member, prompt, purchasedCallback);
        }
        ...
        static function checkPurchaseOfItem(itemMC, itemID, itemType, member, prompt, purchasedCallback) {
            trace("SS: purchase item");
            trace("SS: checkPurchaseOfItem():itemMC:" + itemMC);
            trace("SS: checkPurchaseOfItem()SFX_ITEM_PURCHASE:" + SFX_ITEM_PURCHASE);
            playSound(itemMC, SFX_ITEM_PURCHASE);
            if (member) {
                if (_shell.isMyPlayerMember()) {
                    buyItem(itemID, itemMC, itemType, prompt, purchasedCallback);
        ...
        static function buyItem(itemID, itemMC, itemType, prompt, purchasedCallback) {
            ...
            switch (itemType) {
                case "item" : 
                    ...
                    if (!prompt) {
                        _buyItemDelegate = com.clubpenguin.util.Delegate.create(_this, onInventoryBought, itemMC, itemID, purchasedCallback);
                        _shell.addListener(_shell.BUY_INVENTORY, _buyItemDelegate, _this);
                    }
                    break;
                case "furniture" : 
                    if (prompt) {
                        _interface.buyFurniture(itemID, null, com.clubpenguin.util.Delegate.create(com.clubpenguin.party.BaseParty, checkAddItemPurchasedListener, itemID, itemMC, itemType, purchasedCallback));
                    } else {
                        checkAddItemPurchasedListener(true, itemID, itemMC, itemType, purchasedCallback);
                ...
                case "igloo" : 
                    ...
                    if (!prompt) {
                        _buyIglooDelegate = com.clubpenguin.util.Delegate.create(_this, onIglooBought, itemMC, itemID, purchasedCallback);
        ...
        static function checkAddItemPurchasedListener(doAdd, itemID, itemMC, itemType, purchasedCallback) {
            if (!doAdd) {
                return(undefined);
            }
            switch (itemType) {
                case "item" : 
                    break;
                case "furniture" : 
                    _buyFurnitureDelegate = com.clubpenguin.util.Delegate.create(_this, onFurnitureBought, itemMC, itemID, purchasedCallback);
        ...
        static function onInventoryBought(event, itemMC, itemID, purchasedCallback) {
        ...
        static function onFurnitureBought(event, itemMC, itemID, purchasedCallback) {
        ...
        static function onIglooBought(event, itemMC, itemID, purchasedCallback) {
        ...
        static function onBundleBought(event, itemMC, purchasedCallback) {
