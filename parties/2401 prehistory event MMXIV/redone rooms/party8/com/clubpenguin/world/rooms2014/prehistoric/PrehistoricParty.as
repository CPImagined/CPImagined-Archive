//Created by Action Script Viewer - https://www.buraks.com/asv
    class com.clubpenguin.world.rooms2014.prehistoric.PrehistoricParty
    {
        ...
        static function checkForGarugCongratsMessage(event, puffleId) {
            ...
            var _local_2 = getDinoPuffleQuestIndexFromSubtype(_local_1.attachedPuffle.subTypeID);
            if (_local_2 == -1) {
                return(undefined);
            }
            if (partyCookie[CONSTANTS.DINO_PUFFLE_COLLECTION_KEY][_local_2].state < 4) {
                return(undefined);
            }
            if (partyCookie[CONSTANTS.MESSAGE_KEY][4] == 0) {
        ...
        static function getDinoPuffleQuestIndexFromSubtype(subtype) {
            var _local_1 = 0;
            while (_local_1 < CONSTANTS.DINO_PUFFLE_IDS.length) {
                if (CONSTANTS.DINO_PUFFLE_IDS[_local_1] == subtype) {
                    return(_local_1);
                }
                _local_1++;
            }
        ...
        static function onEatAnimationDone(event) {
            if (_global.getCurrentShell().getMyPlayerId() == event.playerID) {
                var _local_2 = _global.getCurrentShell().getMyPlayerObject().attachedPuffle.id;
                _global.getCurrentEngine().puffleAvatarController.displayPuffleWidget(_local_2, "statsBarWidget", com.clubpenguin.engine.puffles.pufflecarestations.PuffleCareStationsConstants.PUFFLE_EAT_ITEM_ID);
            }
        }
