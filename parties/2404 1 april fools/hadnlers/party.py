...
COOKIE_HANDLER_PARTY = 20240401

COOKIE_HANDLER_NAME = "template"
PARTY_CACHE_CURRENT = "fools24"
...
@handlers.handler(XTPacket(COOKIE_HANDLER_NAME, 'foolsclown'))
async def handle_party_pack(p):
    wig = p.server.items[474]
    suit = p.server.items[247]
    shoes = p.server.items[354]
    await p.add_inventory(item=wig, notify=False, cost=0)
    await p.add_inventory(item=suit, notify=False, cost=0)
    await p.add_inventory(item=shoes, notify=False, cost=0)
    await p.send_xt('foolsclown')
