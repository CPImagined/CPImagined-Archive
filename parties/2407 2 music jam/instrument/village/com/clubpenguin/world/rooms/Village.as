//Created by Action Script Viewer - https://www.buraks.com/asv
    ...
    {
        var _stage, localize, setupNavigationButtons, _INTERFACE, _SHELL, _destroyDelegate, _updateDelegate, wem, _abductionDelegate, aliendAbduction;
        function Village (stageReference) {
            ...
            _stage.triggers_mc.wild_mc.triggerFunction = com.clubpenguin.util.Delegate.create(this, exit, "party14", 389, 264);
		...
		function handleUpdatePlayer(event) {
			_stage.handleUpdatePlayer(event)
		}
        ...
        function init() {
			...
			_SHELL.addListener(_SHELL.ROOM_DESTROYED, _destroyDelegate);
			_SHELL.addListener(_SHELL.PLAYER_FRAME, _updateDelegate);
        	_SHELL.addListener(_SHELL.UPDATE_PLAYER, _updateDelegate);
        	_SHELL.addListener(_SHELL.REMOVE_PLAYER, _updateDelegate);
        	_SHELL.addListener(_SHELL.PLAYER_MOVE, _updateDelegate);
        ...
        function destroy() {
			...
			_SHELL.removeListener(_SHELL.ROOM_DESTROYED, _destroyDelegate);
			_SHELL.removeListener(_SHELL.PLAYER_FRAME, _updateDelegate);
        	_SHELL.removeListener(_SHELL.UPDATE_PLAYER, _updateDelegate);
        	_SHELL.removeListener(_SHELL.REMOVE_PLAYER, _updateDelegate);
        	_SHELL.removeListener(_SHELL.PLAYER_MOVE, _updateDelegate);
            wem.destroy();
            _global.getCurrentAirtower().removeListener("beamup", _abductionDelegate);
            _abductionDelegate = null;
        }
