//Created by Action Script Viewer - https://www.buraks.com/asv
    class com.clubpenguin.engine.avatar.effect.AvatarEffectManager
    {
        ...
        function playEffectForPlayer(effect, player) {
            com.clubpenguin.util.Log.debug("    " + effect);
            for (var _local_3 in effect) {
                trace((("      " + _local_3) + ":") + effect[_local_3]);
            }
            ...
            if (_local_2 != null) {
                ...
                var _local_5 = _ENGINE.getRoomMovieClip();
                var _local_6 = _local_5.customPenguinSize;
                _local_2._xscale = (_local_2._yscale = _local_6);
                var that = this;
                var followUpAnimationStarted = false;
                var killClip = com.clubpenguin.util.Delegate.create(this, endMovieClip, _local_2, effect);
                _local_2.onEnterFrame = function () {
                    if (((!effect.loopAnimation) && (this._currentFrame == this._totalFrames)) && (!followUpAnimationStarted)) {
                        trace("\t\teffect.tweenDuration: " + effect.tweenDuration);
                        if (effect.tweenDuration != undefined) {
                            trace("\t\tq\t1");
                            this.stop();
                            var _local_2 = effect.tweenOptions;
                            _local_2.onComplete = killClip;
                            com.greensock.TweenMax.to(this, effect.tweenDuration, _local_2);
                            trace("\t\tTweenMax: " + com.greensock.TweenMax);
                        } else {
                            trace("\t\tq\t2");
                            that.endMovieClip(this, effect);
                        }
                        followUpAnimationStarted = true;
                    }
        ...
        function endMovieClip(mc, effect) {
            trace(((("\tendMovieClip(mc: " + mc) + ", effect: ") + effect) + ")");
