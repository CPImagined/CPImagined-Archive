//Created by Action Script Viewer - https://www.buraks.com/asv
    class ClientTrigger
    {
        ...
        function ClientTrigger (_shell, _engine) {
            ...
            if (!_shell) {
                trace("ClientTrigger: shell parameter is missing");
                return;
            }
            __shell = _shell;
            if (!_engine) {
                trace("ClientTrigger: engine parameter is missing");
                return;
            }
            __engine = _engine;
        ...
        function addTarget() {
            ...
            if (arguments.length <= 0) {
                trace("addTarget: target movieclip(s) missing");
                return(undefined);
            }
            var _local_3 = 0;
            while (_local_3 < arguments.length) {
                if (typeof(arguments[_local_3]) != "movieclip") {
                    trace(("addTarget: ignoring parameter " + arguments[_local_3]) + " as it's not a movieclip");
                } else {
        ...
        function removeTarget() {
            var _local_5 = 0;
            if (arguments.length <= 0) {
                trace("removeTarget: target movieclip(s) missing");
                return(undefined);
            }
            var _local_4 = 0;
            while (_local_4 < arguments.length) {
                if (typeof(arguments[_local_4]) != "movieclip") {
                    trace(("removeTarget: ignoring parameter " + arguments[_local_4]) + " as it's not a movieclip");
                } else {
        ...
        function countPenguinsOverTargets(_stopOnFirst, _countLocalOnly, _threshold) {
            ...
            if (__targetList.length <= 0) {
                trace("checkPenguinInRoomArea: no targets to check");
                return(0);
            }
            _local_3 = 0;
            _local_11 = new Array();
