package com.clubpenguin.lib.util.data
{
	...
	public class LocalSaveData implements IDisposable,IUniquelyNamable
	{
		...
		public function updateData(_data:Object,_writeNow:Boolean):Boolean
		{
			...
			Log.getLogger(this.getUniqueName()).debug("Updating local data. Saving to disk immediately? " + _writeNow);
		...
		public function clearData(_writeNow:Boolean):Boolean
		{
			...
			Log.getLogger(this.getUniqueName()).debug("Erasing local data. Saving to disk immediately? " + _writeNow);
		...
		public function write():void
		{
			...
			if (!res)
			{
				Log.getLogger(this.getUniqueName()).warn("Failed to save to disk! User disallowed local storage.");
			}
			return res;
		}
