...
async def get_open_igloos(p, igloo_id: int):
    ...
    open_igloos = [await get_igloo_string(igloo) for penguin_id, igloo in p.server.open_igloos_by_penguin_id.items() if penguin_id != p.id and IglooFurniture.query.where((IglooFurniture.igloo_id == igloo.id) & (IglooFurniture.furniture_id == 2179)).gino.first() is not None]
