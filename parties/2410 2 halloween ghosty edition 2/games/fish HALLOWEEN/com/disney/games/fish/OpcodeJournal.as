class com.disney.games.fish.OpcodeJournal
{
   ...
   function checkOpcodeActivity(stamp)
   {
      ...
      if(this.arrStampsSent.length > 0)
      {
         ...
         {
            if(String(this.arrStampsSent[_loc2_]) == stamp)
            {
               _loc3_ == true;
               return true;
            }
   ...
   {
      if(this.checkOpcodeActivity(stampOpcode) == false)
      {
         this.sendStampAction(stampOpcode);
      }
   }
   ...
   function startGame()
   {
      com.disney.dlearning.managers.DLSManager.instance.pushOpcode("start",this.OPCODEGUID_ICEFISHING,this.dlsmCallback,"0");
   }
   function stopGame()
   {
      com.disney.dlearning.managers.DLSManager.instance.pushOpcode("stop",this.OPCODEGUID_ICEFISHING,this.dlsmCallback,"0");
   }
   function sendGameScore(status, score, coins)
   {
      com.disney.dlearning.managers.DLSManager.instance.pushOpcode("gamescore",this.OPCODEGUID_ICEFISHING,this.dlsmCallback,String(score),status,String(coins));
   }
   function sendStampAction(stampOpcode)
   {
      com.disney.dlearning.managers.DLSManager.instance.pushOpcode("selected",stampOpcode,this.dlsmCallback,"0","0","0");
   }
   function sendEventAction(eventOpcode)
   {
      com.disney.dlearning.managers.DLSManager.instance.pushOpcode("selected",eventOpcode,this.dlsmCallback,"0","0","0");
   }
