//Created by Action Script Viewer - https://www.buraks.com/asv
    ...
    {
        var _stage, localize, _SHELL, _PARTY, _destroyDelegate, _rankDelegate, setupNavigationButtons, _triggerWatcher, _triggerWatcherInterval, setupHint, showContent, _ENGINE, _playerHasCards, _INTERFACE;
		...
		function handleGetNinjaRanks(event) {
			...
			if (event.playerID == _SHELL.getMyPlayerId() && event.snowPathRank >= 5) {
		        _SHELL.removeListener(_SHELL.GET_NINJA_RANKS, _rankDelegate, this);
			...
			if (!_PARTY.getMessageViewed(2)) {
				if (event.playerID == _SHELL.getMyPlayerId() && event.cardJitsuRank == 10) {
					_INTERFACE.showContent("w.p2024.ninja.dojo");
				} else {
					_INTERFACE.showContent("w.p2024.ninja.dojo.grasshopper");
				};
			};
		}
