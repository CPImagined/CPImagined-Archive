...
EndParty = datetime(2026, 1, 6, 00, 0, 0)#date[year month day hour minute sec] for end party

CookieHandlerName = "party"
PartyCacheCurrent = "holiday24"
...
async def handle_party_cfc_donate(p, coins: int):
    if p.coins >= coins and 100 <= coins <= 10000:
        ...
        if(donate_player > 0):
            await p.add_inventory(p.server.items[90907], cost=0)
