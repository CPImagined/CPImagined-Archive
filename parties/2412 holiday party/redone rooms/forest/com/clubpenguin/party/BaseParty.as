//Created by Action Script Viewer - https://www.buraks.com/asv
    ...
    {
        static var _shell, _airtower, _interface, _engine, _party, _serverCookieService, _serverCookie, _updatePlayerFrame, _currentParty, _interfaceOverrides;
        function BaseParty () {
        }
        ...
        static function handleUpdatePartySettings(data) {
            ...
            var settings = com.clubpenguin.util.JSONParser.parse(data[1]);
            CURRENT_PARTY.CONSTANTS.PARTY_SERVICE = settings;
        ...
        static function playSound(ui, soundId, volume) {
            ...
            var actionSound = new Sound(ui);
            actionSound.attachSound(soundId);
        ...
        static function get isPartyRoom() {
            ...
            if (((roomID != undefined) && (roomID >= CURRENT_PARTY.CONSTANTS.FIRST_PARTY_ROOM_ID)) && (roomID <= CURRENT_PARTY.CONSTANTS.LAST_PARTY_ROOM_ID)) {
                return(true);
            }
            return(false);
        }
        ...
        static function sendBIIglooVisit(isPartyIgloo, itemId) {
            var context = "party_igloo_visit";
            var action = String("iglooID_" + itemId);
            var game_action_options = new Object();
            game_action_options.message = (isPartyIgloo ? "party_item_present" : "party_item_missing");
            com.clubpenguin.util.TrackerAS2.getInstance().trackGameAction(action, context, game_action_options);
        }
