//Created by Action Script Viewer - https://www.buraks.com/asv
    class com.clubpenguin.shell.PuffleModel
    {
        ...
        static function formatAssetURL(assetURL, puffleColor, puffleSubTypeID) {
            var puffleSubTypeString = ((isNaN(puffleSubTypeID) || (puffleSubTypeID == 0)) ? "" : (String(puffleSubTypeID)));
            assetURL = com.clubpenguin.util.StringUtils.replaceString("%color%", String(puffleColor), assetURL);
            assetURL = com.clubpenguin.util.StringUtils.replaceString("%wildTypeId%", String(puffleSubTypeString), assetURL);
            return(assetURL);
        }
        var id = -1;
        var typeID = -1;
