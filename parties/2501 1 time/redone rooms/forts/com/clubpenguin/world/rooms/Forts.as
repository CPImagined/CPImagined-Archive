//Created by Action Script Viewer - https://www.buraks.com/asv
    ...
    {
        var _stage, _SHELL, localize, setupNavigationButtons, _mascotDelegate, IS_24_HOUR_CLOCK, _now, _secondCheckIntervalID, _destroyDelegate, _checkClockTagetDelegate, showContent, wem, _abductionDelegate, aliendAbduction, _ct_minutes, _ct_hours, _pm_trigger, _ct_day, _today;
        ...
        function init() {
			...
			_mascotDelegate = com.clubpenguin.util.Delegate.create(this, checkForMascot);
            _SHELL.addListener(_SHELL.ADD_PLAYER, _mascotDelegate);
			_SHELL.addListener(_SHELL.REMOVE_PLAYER, _mascotDelegate);
		...
		function checkForMascot() {
			var _loc3_ = _SHELL.getPlayerList();
			var _loc1_ = 0;
			while(_loc1_ < _loc3_.length) {
				var _loc2_ = _loc3_[_loc1_].player_id;
				if(_SHELL.isPlayerMascotById(_loc2_) && _loc2_ == 4) {
					_stage.gary_mc._visible = false;
					return;
				};
				_loc1_ = _loc1_ + 1;
		   };
			_stage.gary_mc._visible = true;
		}
        ...
        function destroy() {
			...
			_SHELL.removeListener(_SHELL.ADD_PLAYER, _mascotDelegate);
			_SHELL.removeListener(_SHELL.REMOVE_PLAYER, _mascotDelegate);
            wem.destroy();
            _global.getCurrentAirtower().removeListener("beamup", _abductionDelegate);
            _abductionDelegate = null;
        }
