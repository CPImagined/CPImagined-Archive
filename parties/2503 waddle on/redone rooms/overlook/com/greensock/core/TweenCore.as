//Created by Action Script Viewer - https://www.buraks.com/asv
    class com.greensock.core.TweenCore
    {
        ...
        function TweenCore (duration, vars) {
            ...
            var _local_2 = ((this.vars.timeline instanceof com.greensock.core.SimpleTimeline) ? (this.vars.timeline) : (((this.vars.useFrames == true) ? (com.greensock.TweenLite.rootFramesTimeline) : (com.greensock.TweenLite.rootTimeline))));
            _local_2.insert(this, _local_2.cachedTotalTime);
        ...
        function setDirtyCache(includeSelf) {
            var _local_2 = ((includeSelf != false) ? this : (timeline));
            while (_local_2) {
        ...
        function setTotalTime(time, suppressEvents) {
            if (timeline) {
                var _local_3 = (cachedPaused ? (cachedPauseTime) : (timeline.cachedTotalTime));
                if (cachedReversed) {
                    var _local_4 = (cacheIsDirty ? (totalDuration) : (cachedTotalDuration));
                    cachedStartTime = _local_3 - ((_local_4 - time) / cachedTimeScale);
                } else {
                    cachedStartTime = _local_3 - (time / cachedTimeScale);
                }
        ...
        function set duration(n) {
            var _local_2 = n / cachedDuration;
            cachedDuration = (cachedTotalDuration = n);
            if ((active && (!cachedPaused)) && (n != 0)) {
                setTotalTime(cachedTotalTime * _local_2, true);
            }
            setDirtyCache(false);
            //return(duration);
        }
