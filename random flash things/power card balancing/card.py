...
async def handle_buy_power_cards(p):
    if p.coins >= 1500:
        power_card = random.choice(p.server.cards.power_cards)
        cards = random.sample(p.server.cards.regular_cards, 5)
        ...
        await p.send_xt('bpc', ','.join([str(card.id) for card in cards]), p.coins)
    else:
